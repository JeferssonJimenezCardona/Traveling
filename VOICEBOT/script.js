document.addEventListener("DOMContentLoaded", () => {
  ...
  // OpenAI API Key (updated from Azure)
  const OPENAI_API_KEY = 
    "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"
  
  // OpenAI API URL (updated from Azure)
  ...
  async function generateAIResponse(input) {
    try {
      ...
      // Updated to use standard OpenAI API format
      const response = await fetch(OPENAI_URL, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${OPENAI_API_KEY}` // Changed from api-key to Authorization Bearer
        },
        body: JSON.stringify({
